    ...
    <b-form v-if="show" @submit="onSubmit">
      ...
      <b-collapse id="collapse-model-config" class="mt-2 mb-2">
        ...
        <b-form-group>
          ...
          >
            <b-form-select
              id="input-optimizer-type"
              v-model="form.model_config.optimizer"
              :options="optimizers"
              required
            />
          </b-form-group>
          ...
          >
            <b-form-select
              id="input-act-fn-type"
              v-model="form.model_config.activation_fn"
...
export default {
  ...
  computed: {
    ...
    datasetVersionState () {
      if (this.form.dataset_version === null || this.form.dataset_version === undefined) {
        return false
      } else {
        return this.form.dataset_version.search('\\s') < 0 && this.form.dataset_version.length >= 1
      }
    },
    nameState () {
  ...
  methods: {
    ...
    async getDatasetsForCodebook () {
      this.datasetsMetadata = await this.$datasetApiClient.list(this.form.cb_name)
      if (this.datasetsMetadata.length >= 1) {
        this.form.dataset_version = this.datasetsMetadata[0].version
      }
    }
  }
}

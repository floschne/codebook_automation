...
from api.model import ModelConfig, TrainingRequest, DatasetMetadata, BooleanResponse
...
def test_train(train_req: TrainingRequest, client: TestClient):
    ...
    response = client.delete("/dataset/remove/",
                             data={"cb_name": train_req.cb_name,
                                   "dataset_version": train_req.dataset_version})
    assert response.status_code == status.HTTP_200_OK
